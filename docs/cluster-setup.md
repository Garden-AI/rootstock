...
### The usage-record spool

`setup-perms` also creates `{cache_root}/usage/` with mode `1777` (sticky, `/tmp`-style). This is the spool for [anonymous per-session usage records](https://github.com/Garden-AI/rootstock#usage-statistics): any user's session must be able to drop a record file, and the sticky bit keeps users from deleting each other's. Records are created `O_EXCL` with unique names, so the directory behaves like `/tmp`, not like a writable code path — but some centers' security scans flag any world-writable directory in project space. If that's a problem on your cluster (or you just don't want usage collection), pass `--no-usage-spool`: a missing spool means collection is off for the install, and `check-perms` treats its absence as a deliberate choice, not an issue. Aggregate with `rootstock usage report`, fold raw records into monthly rollups with `rootstock usage compact`, and send the aggregated counts to the dashboard with `rootstock usage push` (uses the same API credentials as `manifest push`; all three are safe to run from the same cron as the nightly smoke-test).

On clusters where your write access to the install is granted temporarily and later revoked (e.g. Delta), don't put the real spool inside the install — you'd eventually lose the ability to fix its permissions or prune it. Pass `--usage-dir <path>` pointing somewhere you permanently control (a directory under your home works): setup-perms creates the `1777` directory there and symlinks `{cache_root}/usage` to it, so sessions and the collector follow the link without any configuration. Make sure the target's ancestors grant world traverse (`chmod o+x ~` if the spool lives under your home). If the spool ever does become unwritable, user calculations are unaffected — sessions silently skip recording — and `check-perms` will tell you.
