...
Runs MACE in a subprocess, communicating via i-PI protocol.
This measures the overhead of the Rootstock architecture.
"""
...
def benchmark_ipc(
    ...
    """
    Benchmark MACE running in a subprocess via i-PI protocol.
    
    This measures the IPC overhead of the Rootstock architecture.
    ...
    Args:
        ...
        worker_python: Python executable for worker (None = same as current)
    ...
    # We need to handle both installed and development cases
    try:
        from rootstock import RootstockCalculator
    except ImportError:
        # Running from repo root - add to path
    ...
    with RootstockCalculator(
        ...
        # Warmup - includes worker startup, model loading, JIT compilation
...
'''
    ...
    repo_root = Path(__file__).parent.parent
    mock_worker_code = mock_worker_code.format(repo_root=repo_root)
