"""Tests for the MLIPx integration node (rootstock.mlipx.RootstockModel)."""
...
def test_get_spec_shape_and_metadata():
    model = RootstockModel(checkpoint="mace-mp-0-medium", root="/tmp/no_env", device="cpu")
    spec = model.get_spec()
...
def test_get_calculator_reaches_env_boundary():
    ...
    model = RootstockModel(checkpoint="mace-mp-0-medium", root="/tmp/no_env_here", device="cpu")
    with pytest.raises(CheckpointNotFoundError):
        model.get_calculator()


def test_run_rejects_both_cluster_and_root():
    model = RootstockModel(
